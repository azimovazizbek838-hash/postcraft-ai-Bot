...
if not BOT_TOKEN or not OPENAI_API_KEY:
    raise ValueError(
        "BOT_TOKEN va OPENAI_API_KEY environment variable'larda topilmadi!"
    )
...
# Foydalanuvchi tillarini saqlash (Vaqtincha xotirada)
user_languages = {}
...
def get_main_keyboard(lang_name):
    ...
    buttons = [
        [
            KeyboardButton(text=t["btn_generate"]),
...
async def cmd_start(message: types.Message):
    await message.answer(
        "Choose your language / Tilni tanlang:", reply_markup=get_lang_keyboard()
    )
...
@dp.message(
    F.text.in_([t["btn_lang"] for t in TRANSLATIONS.values()])
    | Command("language")
)
...
@dp.message(F.text.in_([t["btn_generate"] for t in TRANSLATIONS.values()]))
    ...
    try:
        response = await openai_client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": f"{t['sys_prompt']} Output response in language code: {t['lang_code']}.",
                },
        ...
        await message.answer(
            ai_message, reply_markup=get_main_keyboard(user_lang)
        )
